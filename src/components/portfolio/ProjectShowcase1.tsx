...
// icon map – add more labels here as needed
...
function ProjectLink({ link, linkLabel }: { link: string; linkLabel: string }) {
  const icon = LINK_ICONS[linkLabel] ?? (
    // fallback: external-link arrow
    <svg
      xmlns="http://www.w3.org/2000/svg"
      width="13"
      height="13"
...
function ProjectCard({ project }: { project: Project }) {
  return (
    <div className="project-card">
      {/* top row: tag + optional link badge */}
      <div className="project-card__header">
        <span className="project-card__tag">{project.tag}</span>
        {project.link && project.linkLabel && (
          <ProjectLink link={project.link} linkLabel={project.linkLabel} />
        )}
      </div>
      ...
      <div className="project-card__metrics">
        {project.metrics.map((m) => (
          <div key={m.k} className="project-card__metric">
            <span className="project-card__metric-value">{m.v}</span>
            <span className="project-card__metric-key">{m.k}</span>
          </div>
        ))}
      </div>
...
// ─────────────────────────────────────────────────────────────────
//  Suggested CSS  (paste into your stylesheet or CSS module)
//  Adjust colors / radii to match your existing design tokens.
// ─────────────────────────────────────────────────────────────────
/*
.project-card {
  display: flex;
  ...
  background: #1a1f2e;          // match your dark card background
}
...
.project-card__metric-value {
  font-size: 0.95rem;
  font-weight: 700;
  color: #5b9cf6;       // accent color – swap to yours
}

.project-card__metric-key {
  font-size: 0.65rem;
  color: #6677889;
